...
export default function GalleryManager() {
  ...
  return <>{role === 'ADMIN' && <PageHeader eyebrow="Keeper's room" title="Gallery manager" subtitle="Upload, organize, and keep your shared chapters safe." action={<button className="primary-button" onClick={() => setOpen(true)}><Plus size={16} /> Upload memory</button>} />}{open && <MemoryUploader onClose={() => setOpen(false)} onSaved={() => window.location.reload()} />}<Gallery /></>;
}
